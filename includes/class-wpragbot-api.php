...
class Wpragbot_API {
    ...
    public function process_chat_request($message, $session_id, $settings) {
        try {
            ...
            error_log('WPRAGBot: Settings check - API Key: ' . (empty($settings['gemini_api_key']) ? 'MISSING' : 'SET') . 
            ...
            $query_embedding = $this->generate_embedding($message, $settings['gemini_api_key'], 'retrieval_query');
     ...
     */
    private function generate_embedding($text, $api_key, $task_type = 'retrieval_document') {
        if (empty($api_key)) {
            return new WP_Error('missing_api_key', 'Gemini API key is required');
        }
        ...
        if (is_wp_error($response)) {
            return new WP_Error('embedding_request_failed', 'Failed to connect to Gemini API: ' . $response->get_error_message());
        }
        ...
        if ($response_code !== 200) {
            ...
            return new WP_Error('embedding_api_error', 'Gemini API error: ' . $error_message, array('status' => $response_code));
        }
     ...
     */
    private function generate_response($message, $context, $api_key, $system_prompt, $collection_name = '') {
        if (empty($api_key)) {
            return new WP_Error('missing_api_key', 'Gemini API key is required');
        }
        ...
        if (is_wp_error($response)) {
            error_log('WPRAGBot: Failed to connect to Gemini API: ' . $response->get_error_message());
            return new WP_Error('generation_request_failed', 'Failed to connect to Gemini API: ' . $response->get_error_message());
        }

        $response_code = wp_remote_retrieve_response_code($response);
        $response_body = wp_remote_retrieve_body($response);

        error_log('WPRAGBot: Gemini response code: ' . $response_code);
        error_log('WPRAGBot: Response body length: ' . strlen($response_body) . ' bytes');

        if ($response_code !== 200) {
            $error_data = json_decode($response_body, true);
            $error_message = isset($error_data['error']['message']) ? $error_data['error']['message'] : 'Unknown error';
            error_log('WPRAGBot: Gemini API error: ' . $error_message);
            return new WP_Error('generation_api_error', 'Gemini API error: ' . $error_message, array('status' => $response_code));
        }

        $data = json_decode($response_body, true);

        if (!isset($data['candidates'][0]['content']['parts'][0]['text'])) {
            error_log('WPRAGBot: Invalid response structure from Gemini API');
            ...
            return new WP_Error('invalid_generation_response', 'Invalid response from Gemini API');
        }
